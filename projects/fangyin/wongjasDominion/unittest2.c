...
int main(){
  ...
  printf("hand count = %d, expected = %d\n", testc.handCount[currentPlayer], state.handCount[currentPlayer] + newCards - discarded);
	printf("deck count = %d, expected = %d\n", state.deckCount[currentPlayer], testc.deckCount[currentPlayer] - 1);
  printf("numActions = %d, expected = %d\n", state.numActions, testc.numActions + 1);
	printf("coins = %d, expected = %d\n", state.coins, testc.coins + xtraCoins);
	assertPass(testc.handCount[currentPlayer], state.handCount[currentPlayer] + newCards - discarded);
	assertPass(testc.deckCount[currentPlayer],state.deckCount[currentPlayer]);
